...
class RCloudApi {
    ...
    private static $appSecret;             //secret
    private static $serverApiUrl = 'https://api.cn.rong.io';    //请求服务地址
    private static $format;                //数据格式 json/xml
     ...
     * @param string $format
     */
    private function __construct($appKey,$appSecret,$format = 'json'){
        self::$appKey = $appKey;
    ...
    public static function getInstance($appKey, $appSecret) {
        if (!is_object(self::$instance)) {
            self::$instance = new self($appKey, $appSecret);
        }
        self::$header = self::createHttpHeader();
     ...
     * @return bool
     */
    private static function createHttpHeader() {
        $nonce = mt_rand();
        $timeStamp = time();
    ...
    private function curl($url, $params) {
        $url = self::$serverApiUrl . $url . self::$format;
        $httpHeader = self::$header;
        $arrRes = Base::curl($url, $params, $httpHeader);
